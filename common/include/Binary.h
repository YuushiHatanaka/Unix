...
public:
    ...
    // 取得
    //-----------------------------------------------------------------------------
    void Get(u_char* data, size_t size)
    {
        // データ設定
        this->Get(data, 0, size);
    }

    //-----------------------------------------------------------------------------
    // 取得
    //-----------------------------------------------------------------------------
    void Get(u_char* data, size_t offset, size_t size)
    {
        // データ設定
        memcpy(data, &(this->m_data[offset]), size );
    }
    ...
    Binary Sub(int start)
    {
        ...
        // 文字数分繰り返し
        int _count=0;
        for(size_t i=start; i<this->m_size; i++, _count++ )
        {
    ...
    // 置換
    //-----------------------------------------------------------------------------
    void Replace(size_t offset, u_char* data, size_t size)
    {
        // NULL判定
        if(this->m_data == NULL)
        {
        ...
        // バイナリ化
        Binary _data(data, size);

        // 置換
        this->Replace(offset, _data);
    }

    //-----------------------------------------------------------------------------
    // 置換
    //-----------------------------------------------------------------------------
    void Replace(size_t offset, Binary& data)
    {
        // NULL判定
        if(this->m_data == NULL)
        {
